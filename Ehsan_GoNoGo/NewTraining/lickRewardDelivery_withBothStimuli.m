...
% the day  
prompt = {'Enter the mouse number:','Enter the box number:','Enter the mouse session number:','Enter the trial numbers:','No Lick Time?(0-1):'}; 
titleBox = 'Input';
dims = [1 35]; 
...
NoLickTime = str2num(dialogBoxInputs{5});
...
preferredStimDuration = 3; %in sec
...
%         end
        ...
        if lickFlag == 1
            ...
            disp(['False Alarm: ', num2str(FACounter), ' / ', num2str(NonPreferredStimCounter)]);
            disp(['passed time: ',num2str(floor((GetSecs()-startRecTime)/60)), ' Minutes']);
            
            
        else
            CRCounter = CRCounter + 1;
            disp(['Correct Rejection: ', num2str(CRCounter), ' / ', num2str(NonPreferredStimCounter)]);
            disp(['passed time: ',num2str(floor((GetSecs()-startRecTime)/60)), ' Minutes']);
